...
@export var stats_module: RMOD_STATS
var teamsides: Team

func update_indicator():
	if stats_module.state == stats_module.get_child(stats_module.states.OFF().name):
		$Label.text= "OFF"
	elif stats_module.state == stats_module.get_child(stats_module.states.ON().name):
		$Label.text= str(stats_module.health)
	elif stats_module.state == stats_module.get_child(stats_module.states.DEAD().name):
		$Label.text= "DEAD"
...
#func use_skill(target,target_loc): #opsional Update Afif
	...
	#robotAction = ROBOT_ACTION.SKILL
			...
			#self.on_defensed(defenseValue)
				...
				#Global.battle.control.highlight.skill_zone_enable_center(target_loc,skillRange)
				#for i in Global.battle.control.highlight.get_used_cells():
					#var tempTarget = Global.battle.tile_data[i]	#team system
					#if is_instance_of(tempTarget,Robot) and tempTarget.get_parent().name == get_parent().name:
						#tempTarget.on_healed(healingValue)
			#else:
				#target.on_healed(healingValue)
		#if attack_skill :
			#if ranged_skill:
				##Global.battle.control.highlight.skill_zone_enable_center(target_loc,skillRange) #request highlight zone at highlight system
				#for i in ControlCore.UIS.highlight_tmas.get_used_cells(): #GBO tak punys highlighttm_ref
					##print(i)
					#var tempTarget = ControlCore.tile_data[i] #target system
					#if is_instance_of(tempTarget,Robot) and tempTarget.get_parent().name != get_parent().name:
						#$skill.play()
						#$AnimatedSprite2D.position = ControlCore.GameObject.arenatm_ref.map_to_local(tempTarget.robot_map_loc) - position
						#$AnimatedSprite2D.show()
						#$AnimatedSprite2D.play("Skill")
						#tempTarget.on_damaged(skillDamage)
						#await get_tree().create_timer(1.0).timeout
			...
			#self.on_defensed(defenseValue)
	#await get_tree().create_timer(0.5).timeout
	##Global.battle.control.highlight.clear() #clear highlight
	#robotAction = ROBOT_ACTION.IDLE
...
#func attack_obstacle(target:Obstacle,with_skill:bool):
		...
		#robotAction = ROBOT_ACTION.ATTACK
		#$AnimatedSprite2D.show()
		#$AnimatedSprite2D.position = ControlCore.GameObject.arenatm_ref.map_to_local(target.pos_in_map) - position - Vector2(0,32)
		#$AnimatedSprite2D.play("Attack")
		#await get_tree().create_timer(1.0).timeout
		#$AnimatedSprite2D.hide()
		#$AnimatedSprite2D.position = position
...
#func on_deployed():
	...
	#await get_tree().create_timer(1.0).timeout
	#$AnimatedSprite2D.hide()
	#$AnimatedSprite2D.position = position - ControlCore.GameObject.arenatm_ref.position
